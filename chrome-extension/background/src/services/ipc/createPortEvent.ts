...
export function createPortEvent(): Event<{ port: Port; id: string }> {
  ...
  chrome.runtime.onConnect.addListener(async (port: chrome.runtime.Port) => {
    if (port.name.startsWith('crx-mcp-over-cdp-side-panel:service-worker:')) {
      // Split on first two colons to get everything after 'crx-mcp-over-cdp-side-panel:service-worker:'
