...
import { CRX_MCP_OVER_CDP_SIDE_PANEL_HELLO, CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, CRX_MCP_OVER_CDP_SIDE_PANEL_RECONNECT } from '@shared/utils/message';
import {
  ChannelClient,
  ChannelServer,
  IChannel,
...
function createMessageEvent(
  ...
  eventName: string = CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE,
  socket: ISocketLike,
): Event<VSBuffer> {
...
export class RemoteMessageClient<TContext extends string>
  ...
  private createProtocol(): Protocol {
    ...
    const onReconnect = createMessageEvent(
      this.source,
      this.target,
      CRX_MCP_OVER_CDP_SIDE_PANEL_RECONNECT,
      this.socket,
    );

    // This is already an Event<VSBuffer> so it matches the expected type
    const onMessage = createMessageEvent(this.source, this.target, CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, this.socket);
  ...
  private sendHelloMessage(): void {
    const helloMessage = {
      type: CRX_MCP_OVER_CDP_SIDE_PANEL_HELLO,
      source: this.source,
      target: this.target,
      body: [],
    };

    this.socket.emit(CRX_MCP_OVER_CDP_SIDE_PANEL_HELLO, helloMessage);
  }
