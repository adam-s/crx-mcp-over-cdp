...
import { CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, CRX_MCP_OVER_CDP_SIDE_PANEL_DISCONNECT } from '@shared/utils/message';
...
export class RemoteProtocol implements IMessagePassingProtocol {
  ...
  send(message: VSBuffer): void {
    ...
    this.socket.emit(CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, {
  ...
  async disconnect(): Promise<void> {
    ...
    this.socket.emit(CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, {
      type: CRX_MCP_OVER_CDP_SIDE_PANEL_DISCONNECT,
      source: this.source,
      target: this.target,
    });
  }
