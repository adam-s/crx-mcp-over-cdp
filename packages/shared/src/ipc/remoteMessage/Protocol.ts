...
import { Message, CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE } from '@shared/utils/message';
...
export class Protocol implements IMessagePassingProtocol {
  ...
  constructor(
    ...
    this._eventListeners.set(CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, messageListener);

    // Register the listener
    this._socket.on(CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, messageListener);
  ...
  private sendViaSocket(payload: Message): void {
    try {
      ...
      this._socket.emit(CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE, payload);
    } catch (error) {
  ...
  private createMessagePayload(message: VSBuffer): Message {
    return {
      type: CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE,
      source: this.source,
