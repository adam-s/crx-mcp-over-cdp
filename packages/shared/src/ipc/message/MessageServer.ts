...
// Local imports
import { CRX_MCP_OVER_CDP_SIDE_PANEL_DISCONNECT, CRX_MCP_OVER_CDP_SIDE_PANEL_HELLO, CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE } from '@shared/utils/message';
...
export class MessageServer extends IPCServer {
  ...
  private static createClientConnectionHandler(channelId: string): Event<ClientConnectionEvent> {
    ...
    return Event.map(filteredHello, ({ sender, source }) => {
      ...
      const disconnectEvent = createMessageEvent(source, channelId, CRX_MCP_OVER_CDP_SIDE_PANEL_DISCONNECT);
      const onDidClientDisconnect = Event.any(Event.signal(disconnectEvent), onReconnect.event);
      const onMessage = createMessageEvent(source, channelId, CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE);
