...
export class MessageClient<TContext extends string>
  ...
  private createProtocol(): Protocol {
    ...
    const onReconnect = createMessageEvent(this.source, this.target, CRX_MCP_OVER_CDP_SIDE_PANEL_RECONNECT);
    const onMessage = createMessageEvent(this.source, this.target, CRX_MCP_OVER_CDP_SIDE_PANEL_MESSAGE);
